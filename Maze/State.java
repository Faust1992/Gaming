...
public class State {
	...
	public ArrayList<State> getSuccessors(boolean[][] explored, Maze maze) {
        ...
        if (!explored[this.square.X][this.square.Y-1] && maze.getSquareValue(this.square.X, this.square.Y-1) != '%')
          array.add(new State(new Square(this.square.X, this.square.Y-1), this, gValue+1, depth+1));
        if (!explored[this.square.X+1][this.square.Y] && maze.getSquareValue(this.square.X+1, this.square.Y) != '%')
            array.add(new State(new Square(this.square.X+1, this.square.Y), this, gValue+1, depth+1));
        if (!explored[this.square.X][this.square.Y+1] && maze.getSquareValue(this.square.X, this.square.Y+1) != '%')
            array.add(new State(new Square(this.square.X, this.square.Y+1), this, gValue+1, depth+1));
        if (!explored[this.square.X-1][this.square.Y] && maze.getSquareValue(this.square.X-1, this.square.Y) != '%')
            array.add(new State(new Square(this.square.X-1, this.square.Y), this, gValue+1, depth+1));
