...
public class shi86Player extends Player {
    ...
    private int sbe(GameState state)
    {
    	...
    	if (state.status() == -1)
    		...
    		for (int i = 7; i < 13; i++)
    		{
    			a = state.stoneCount(i);
    			if (a+i < 13 && state.stoneCount(a+i) == 0 && state.stoneCount(12-a-i) > 0)
    				sbevalue -= (state.stoneCount(12-a-i)-1);
    	}
    	}
    	return sbevalue;
    }
